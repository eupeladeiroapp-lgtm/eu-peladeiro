...
export default function JogoConvite() {
  ...
  function handleShareWhatsApp() {
    const url = window.location.href
    const data = jogo ? formatDate(jogo.data_hora) : ''
    const hora = jogo ? formatTime(jogo.data_hora) : ''
    const local = jogo?.local ? ` — ${jogo.local}` : ''
    const text = `⚽ *${grupo?.nome || 'Pelada'}*\n📅 ${data} às ${hora}${local}\n\nConfirma presença? ${url}`
      ...
      <div className="px-5 mt-5">
        ...
        <div className="flex gap-2 mt-3">
          <button
            ...
            className="flex-1 flex items-center justify-center gap-2 bg-[#25D366] text-white font-semibold py-3.5 rounded-xl hover:bg-[#1ebe5d] transition-colors"
          >
          ...
          <button
            onClick={handleShare}
            className="flex items-center justify-center gap-2 px-4 bg-white border-2 border-gray-200 text-gray-700 font-semibold py-3.5 rounded-xl hover:bg-gray-50 transition-colors"
          >
            <Share2 size={18} />
          </button>
        </div>
