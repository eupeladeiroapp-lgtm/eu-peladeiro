import { Calendar, Check, MapPin, Share2, Users, X } from 'lucide-react'
  ...
  async function fetchJogo() {
    ...
    try {
      ...
      if (jogoData) {
        ...
        setConfirmacoes((confsData as Confirmacao[]) || [])
      }
    } catch (err) {
      console.error(err)
          ...
          <div className="space-y-3 mb-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-verde-campo font-bold">{formatTime(jogo.data_hora)}</p>
              </div>
            </div>
            ...
            <div className="flex items-center gap-3">
              ...
              <div className="flex-1">
                ...
                <div className="flex items-center gap-2">
                  <AvatarStack avatars={confirmados.map(() => null)} names={confirmados.map(() => 'Jogador')} />
              ...
              <div className="flex gap-3">
                ...
                {userStatus !== 'recusado' && (
                  <button
                    ...
                    disabled={saving}
                    className={`flex items-center justify-center gap-2 font-bold py-4 rounded-xl transition-colors disabled:opacity-60 ${
                      userStatus === 'confirmado'
                        ? 'flex-1 bg-red-50 text-red-500 border-2 border-red-200 hover:bg-red-100'
                        : 'px-4 bg-gray-100 text-gray-500 hover:bg-gray-200'
                    }`}
                  >
                    <X size={20} />
                    {userStatus === 'confirmado' ? 'Cancelar' : 'Recusar'}
                  </button>
                )}
              </div>
            )}
